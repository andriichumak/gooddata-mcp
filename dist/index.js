...
const backend = tigerFactory().onHostname(HOST).withAuthentication(new TigerTokenAuthProvider(TOKEN));
const workspace = backend.workspace(WORKSPACE);
// @ts-ignore
const client = backend.decorated.client;
...
//     },
// );
server.tool("visualization_search", "Find relevant visualizations by search term", { search_term: z.string() }, async ({ search_term }) => {
    const searchResults = await workspace.genAI()
        .getSemanticSearchQuery()
        .withQuestion(search_term)
        .withDeepSearch(true)
...
//     },
// );
server.tool("visualization_png_export", "Export a visualization to image", { visualizationId: z.string() }, async ({ visualizationId }) => {
